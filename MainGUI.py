...
class MainGUI():
    ...
    def CopyFile(self, path_source, path_target):
        try:
            ...
            self.copy_progress.Disable()
            self.copy_progress.Destroy()
            self.wnd.ErrorMessage(u"Возникла ошибка при копировании файлов. Файл может быть занят другой программой. Или, возможно, стоит запустить программу от имени администратора.")
    ...
    def CreateCopy(self, event = None):
        ...
        if os.path.exists(os.getenv("APPDATA") + "\\Cenchanced\\CLE_backup.dtd"):
            dlg = wx.MessageDialog(self.wnd, u"Программа обнаружила уже созданную ранее резервную копию. Заменить её на новую?", u"Замена резервной копии?", wx.YES_NO | wx.ICON_QUESTION)
        ...
        try:
            ...
            t = threading.Thread(target=self.CopyFile, args=(gl.cfg["DatPath"], os.getenv("APPDATA") + "\\Cenchanced\\CLE_backup.dtd", ))
            t.daemon = True
            t.start()
        except:
            self.wnd.ErrorMessage(u"Возникла ошибка при копировании файлов. Возможно, стоит запустить программу от имени администратора.")
    ...
    def RestoreFromCopy(self, event = None):
        ...
        try:
            ...
            self.copy_progress.UpdateStageText(u"Копируем файлы. Пожалуйста, подождите...")
            t = threading.Thread(target=self.CopyFile, args=(os.getenv("APPDATA") + "\\Cenchanced\\CLE_backup.dtd", gl.cfg["DatPath"], ))
            t.daemon = True
            t.start()
        except:
            self.wnd.ErrorMessage(u"Возникла ошибка при копировании файлов. Возможно, стоит запустить программу от имени администратора.")
    ...
    def MakeLogs(self, event = None):
        ...
        for root, dirs, files in os.walk(os.getenv('APPDATA') + "/Cenchanced"):  # Список всех файлов и папок в директории folder
            for file in files:
