...
def PatchOneString(file_id, gossip_id, content, args_order, dat_file):
    ...
    con.commit()
    cur.execute(
        'INSERT INTO text_files (file_id, gossip_id, content, args_order) VALUES("' + file_id + '", "' + gossip_id + '", "' + content + '", "' + args_order + '")')
    con.commit()
    con.close()
    PatchFile(dat_file)
...
def AddOneStringToPatch(file_id, gossip_id, content, args_order):
    ...
    try:
        ...
        print(
            'INSERT INTO text_files (file_id, gossip_id, content, args_order) VALUES("' + file_id + '", "' + gossip_id + '", "' + ''.join(
                content) + '", "' + args_order + '")')
