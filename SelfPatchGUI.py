...
class SelfPatchGUI():
    ...
    def Start(self, event=None):
        ...
        self.text_args_order = wx.StaticText(self.wnd.gui_panel, -1, u"Порядок аргументов: ", pos=(15, 40),
                                             size=(130, -1))
        ...
        self.input_value_id = wx.TextCtrl(self.wnd.gui_panel, -1, "", pos=(80, 68), size=(340, 130),
        ...
        self.cb1 = wx.CheckBox(self.wnd.setting_panel,
                               label=u'Использовать стиль оформления \nпорядка аргументов в тексте\n(как на сайте)',
                               pos=(5, 10))
    ...
    def OnClickSelfPatchButton(self, event):
        if self.new_argument_type == "True":
            if not (
                    self.input_file_id.GetValue() and self.input_gossip_id.GetValue() and self.input_value_id.GetValue()):
